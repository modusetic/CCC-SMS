...
function buildSystemPrompt(organizerName, contactName, directorAlternatives = [], timezone, settings = {}, context = {}) {
  ...
  const backupSection = directorAlternatives.length > 0
    ? ` The organizer has communicated these available times: ${directorAlternatives.join('; ')}. Offer them if the contact declines the primary options.`
    : '';
...
{"approved":true or false,"contactMsg":"<friendly SMS under ${maxLen} chars to ${contactName}; if approved, must explicitly state the confirmed time '${pendingContactSuggestion}'>","organizerAck":"<friendly SMS under ${maxLen} chars to ${organizerName}; if approved, must explicitly state the confirmed time '${pendingContactSuggestion}'>"}`;
