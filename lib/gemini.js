...
function buildSystemPrompt(organizerName, contactName, directorAlternatives = [], timezone, settings = {}, context = {}) {
  ...
  const { proposedTimes = [], lastContactMsg = null } = context;

  const backupSection = directorAlternatives.length > 0
    ? ` The organizer has communicated these available times: ${directorAlternatives.join('; ')}. Offer them if the contact declines the primary options. IMPORTANT: If the contact accepts or agrees to any of these organizer-communicated times, return the confirmed JSON immediately — do not treat it as a counter-proposal.`
    : '';
  const proposedSection = proposedTimes.length > 0
    ? ` You have proposed these times to ${contactName}: ${proposedTimes.join(', ')}. Only offer dates/times consistent with what the organizer has communicated — do not invent new dates.`
    : '';
  const sentSection = lastContactMsg
    ? ` Your most recent message to ${contactName} was: "${lastContactMsg}". Stay consistent with this.`
    : '';

  return `You are a friendly scheduling assistant named ${name} working on behalf of ${organizerName}. Your job is to find a meeting time that works for ${contactName} via SMS. Keep all messages under ${maxLen} characters. ${tone} Today's date is ${today} (timezone: ${tz}).${backupSection}${proposedSection}${sentSection}
...
async function getNextReply(thread, incomingMessage, settings = {}) {
  ...
  const model = genAI.getGenerativeModel({
    ...
    systemInstruction: buildSystemPrompt(
      ...
      { proposedTimes: thread.proposedTimes || [], lastContactMsg }
    )
  });
...
async function getOrganizerApprovalDecision(organizerName, contactName, pendingContactSuggestion, organizerMessage, settings = {}, context = {}) {
  const maxLen = settings.maxMessageLength || 160;
  const { directorAlternatives = [] } = context;
  ...
  const altSection = directorAlternatives.length > 0
    ? `\nOther times the organizer has indicated as available: ${directorAlternatives.join('; ')}.`
    : '';
...
async function getOrganizerUpdateReply(organizerName, contactName, organizerMessage, settings = {}, context = {}) {
  const name   = settings.assistantName    || 'Alex';
  const maxLen = settings.maxMessageLength || 160;
  const { proposedTimes = [], directorAlternatives = [], lastContactMsg = null } = context;
  ...
  const contextLines = [];
  if (proposedTimes.length > 0) contextLines.push(`Originally proposed times: ${proposedTimes.join(', ')}.`);
  if (directorAlternatives.length > 0) contextLines.push(`The organizer has previously communicated: ${directorAlternatives.join('; ')}.`);
